...
namespace TerraFirmaLike.Utility
{
    class AreaMethods
    {
        ...
        public static bool IsBeamNearby(IWorldAccessor world, BlockPos pos, BlockPos[] cardinalOffsets, BlockPos[] largeAreaBelowOffsets, bool checkSide)
        {
            var bA = world.BlockAccessor;
            Block block = bA.GetBlock(pos);
            Block cBlock = block;
            int count = 0;
            Block dBlock = bA.GetBlock(pos.X, pos.Y - 1, pos.Z);
            if (dBlock.HasBehavior<BehaviorSupportBeam>()) return true;

            foreach (var v in cardinalOffsets)
            {
                cBlock = bA.GetBlock(pos.X + v.X, pos.Y + v.Y, pos.Z + v.Z);
                if (cBlock.HasBehavior<BehaviorSupportBeam>() && checkSide)
                {
                    return false;
                }
                if (cBlock.IsReplacableBy(block))
                {
                    count++;
                }
            }

            if (count >= cardinalOffsets.Count())
            {
                return false;
            }

            foreach (var v in largeAreaBelowOffsets)
            {
                cBlock = bA.GetBlock(pos.X + v.X, pos.Y + v.Y, pos.Z + v.Z);
                if (cBlock.HasBehavior<BehaviorSupportBeam>())
                {
                    return true;
                }
            }
